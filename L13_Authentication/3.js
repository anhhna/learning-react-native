...
export default class l13_authentication extends Component {
  ...
  sendChat(){
    console.log("Chat will be sent:"+ this.state.chat);
    if (this.state.chat && this.state.chat.length > 0){
      this.chatRef.transaction((chats) => {
        //We need to transform the chats object to what we want.
        if (!chats){
          chats = [];
        }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          <TextInput style={styles.textInput} value={this.state.chat}
          onChangeText={(t)=>{
            this.setState({chat:t})
          }}>
          </TextInput>
